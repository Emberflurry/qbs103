...
2. The "margin" i.e. if you want it to perform this function on rows or columns."**MARGIN = 1**" indicates it should perform the function on every on every row, 
resulting in an array of **m length** and
 "**MARGIN = 2**" indicates it should perform the function on every column, 
 resulting in an array of **n length**.
...
1. Write a function to calculate the 
relative abundance of each miRNA in each sample 
(i.e. what percentage of the total miRNA in a given sample is your miRNA of interest).
 Verify that each sample has a total relative abundance of 1.
```{r}
#is a sample a subject? or group of subjects? or our total dataset?
relabund = function (sample,df) {
  #get relabund for ea miRNA - ie for a single subject (col), give row1 val/sum of all row vals for col
  vals = df[[sample]]
  colsum = sum(df[[sample]])
  # print(colsum)
  ra_vals = vals/colsum
  return(ra_vals)
...
mysample = 'Subject1'
relabund(mysample,miRNA)

#TODO check relabund sums to 1
sum(relabund('Subject1', miRNA))   # should be 1 (or super close)
sum(miRNA$Subject1) #for checking
```
2. Using the apply function, identify:
 the highest relative abundance each miRNA has in a single sample.
```{r}
#simpler way to get new relative abundance dataframe:
miRNA_rel_abund <- as.data.frame(lapply(miRNA, function(col) col/sum(col)))

#miRNA_rel_abund: miRNAs as rows, samples(Subject_) as columns
max_rel_abund <- apply(X=miRNA_rel_abund, MARGIN=1, FUN=max)

head(max_rel_abund)
#prev attempts:
# gethighest = function(ras){
#   hi = sort(ras,decreasing=TRUE)
#   return(hi)
# }

# r1 = apply(miRNA,MARGIN=1,FUN=relabund)

```
3. Sort the dataset by miRNA with the highest relative abundance 
and generate a heatmap of the relative abundance 
(not the absolute counts) of each miRNA, 
including the top 20 miRNA by single-sample relative abundance.
```{r}
#Add the max_rel_abund as a column for sorting, 
#then can remove after sorting
rownames(miRNA_rel_abund) <- rownames(miRNA) #keep post-lapply 
miRNA_rel_abund$max_rel_abund <- max_rel_abund
miRNA_rel_abund_sorted <- miRNA_rel_abund[order(-miRNA_rel_abund$max_rel_abund), ] #dec
#sel top 20:
miRNA_top20 <- miRNA_rel_abund_sorted[1:20, -ncol(miRNA_rel_abund_sorted)] 
#^ removes the 'max_rel_abund' column for cleanshi

# install.packages("pheatmap")
library(pheatmap)

pheatmap(miRNA_top20,
         cluster_rows=FALSE,
         cluster_cols=FALSE,
         main="Top 20 miRNAs by Max Relative Abundance",
...
4. Generate a random binary variable for sex and a categorical variable for age group
 using distributions and age cutoffs (hint: use the *cut()* function) of your chosing.
  Add tracking bars to your plot for your generated variables.
```{r}
set.seed(42)
sample_names <- colnames(miRNA)  #samples are columns of og miRNA data
Sex <- sample(c("Male", "Female"), size=length(sample_names), replace=TRUE)

Age <- round(runif(length(sample_names), min=18, max=70)) 
#arb age cutoffs:
AgeGroup <- cut(Age, breaks=c(0, 30, 50, 100), labels=c("Young", "Middle", "Older"), right=FALSE)
#combine the data:
...
# Now pass this as annotation_col
library(pheatmap)
pheatmap(miRNA_top20,
         cluster_rows=FALSE,
         cluster_cols=FALSE,
...
```{r}
#wanted to reorder columns by age, sex for viz:Age
# Assuming you already have sample_metadata from before
sample_metadata$Sample <- factor(sample_metadata$Sample, levels=sample_names)  # ensure factor with correct order

# Arrange by AgeGroup and then by Sex
library(dplyr)
sample_metadata_ordered <- sample_metadata %>%
  arrange(AgeGroup, Sex)

# Get new order of sample (column) names
new_order <- as.character(sample_metadata_ordered$Sample)
miRNA_top20_ordered <- miRNA_top20[, new_order]
annotation_ordered <- annotation[new_order, , drop=FALSE]
library(pheatmap)
pheatmap(miRNA_top20_ordered,
         cluster_rows=FALSE,
         cluster_cols=FALSE,
